...
const hitSound = new Audio('/sounds/hit-something.mp3')
...
const obstacle_pool = []
for (let i = 0; i < 10; i += 2) {
    let sp1 = new THREE.Mesh(cube, new THREE.MeshStandardMaterial())
    sp1.material.metalness = 0.4
    sp1.material.roughness = 0.7
    sp1.position.set(Math.random() * 30, 0, Math.random() * (-100))
    sp1.material.color = new THREE.Color('brown')

    let sp2 = new THREE.Mesh(cube, new THREE.MeshStandardMaterial())
    sp2.material.metalness = 0.4
    sp2.material.roughness = 0.7
    sp2.position.set(Math.random() * 30, 0, Math.random() * (-100))
    sp2.material.color = new THREE.Color('brown')

    obstacle_pool[i] = sp1
    obstacle_pool[i + 1] = sp2
    scene.add(sp1, sp2)
}
...
function ExplodeAnimation(x, y, color, score) {
    score.hits++;
...
parts.push(new ExplodeAnimation(0, 0, 0xffffff, game_score));

//
const updateObjects = (object_pool, obstacle_pool, delta, x, spaceship, score) => {
    for (let i = 0; i < 10; i += 2) {
        ...
        if (obstacle_pool[i].position.z >= 0) {
            obstacle_pool[i].position.x = Math.random() * (30 - x)
            obstacle_pool[i].position.z = Math.random() * (-100) - 30
        } else {
            obstacle_pool[i].position.z += delta
        }
        ...
        if (obstacle_pool[i + 1].position.z >= 0) {
            obstacle_pool[i + 1].position.x = Math.random() * (x - 30)
            obstacle_pool[i + 1].position.z = Math.random() * (-100) - 30
        } else {
            obstacle_pool[i + 1].position.z += delta
        }
        ...
        if (spaceship.position.x <= object_pool[i].position.x + 1 && spaceship.position.x >= object_pool[i].position.x - 1) {
            ...
            if (spaceship.position.z <= object_pool[i].position.z + 1 && spaceship.position.z >= object_pool[i].position.z - 1) {
                console.log(score);
                playLaserSound()
                parts.push(new ExplodeAnimation(spaceship.position.x, spaceship.position.y, object_pool[i].material.color, game_score));
            }
        }
        if (spaceship.position.x <= object_pool[i + 1].position.x + 1 && spaceship.position.x >= object_pool[i + 1].position.x - 1) {
            ...
            if (spaceship.position.z <= object_pool[i + 1].position.z + 1 && spaceship.position.z >= object_pool[i + 1].position.z - 1) {
                console.log(score);
                playLaserSound()
                parts.push(new ExplodeAnimation(spaceship.position.x, spaceship.position.y, object_pool[i + 1].material.color, game_score));
            }
        }

        if (spaceship.position.x <= obstacle_pool[i].position.x + 1 && spaceship.position.x >= obstacle_pool[i].position.x - 1) {
            //console.log("spaceship position: "+spaceship.position.x)
            if (spaceship.position.z <= obstacle_pool[i].position.z + 1 && spaceship.position.z >= obstacle_pool[i].position.z - 1) {
                console.log(score);
                playHitSound();
                game_score.crashes++;
            }
        }
        if (spaceship.position.x <= obstacle_pool[i + 1].position.x + 1 && spaceship.position.x >= obstacle_pool[i + 1].position.x - 1) {
            //console.log("spaceship position: "+spaceship.position.x)
            if (spaceship.position.z <= obstacle_pool[i + 1].position.z + 1 && spaceship.position.z >= obstacle_pool[i + 1].position.z - 1) {
                console.log(score);
                playHitSound()
                game_score.crashes++;
            }
...
const animate = () => {
    document.getElementById('hit').innerHTML = "hits: " + Math.floor(game_score.hits / 3);
    document.getElementById('crashes').innerHTML = "crashes: " + Math.floor(game_score.crashes / 3);
    ...
    updateObjects(object_pool, obstacle_pool, delta * 40, spaceship.position.x, spaceship, game_score)
